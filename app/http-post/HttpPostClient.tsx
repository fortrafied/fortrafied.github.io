...
export default function HttpPostClient() {
    ...
    try {
      ...
      if (res.ok) {
        setResult({
          ...
          body: `Status: ${res.status}\nThe data was transmitted via HTTP POST. Check your DLP console for alerts.\n\nResponse:\n${JSON.stringify(data, null, 2)}`,
        });
      } else {
        setResult({
          visible: true,
    ...
    try {
      ...
      if (res.ok) {
        setUploadResult({
          ...
          body: `Status: ${res.status}\nFiles were transmitted via HTTP POST. Check your DLP console for alerts.\n\nResponse:\n${JSON.stringify(data, null, 2)}`,
        });
        setFiles([]);
      } else {
        setUploadResult({
    ...
    <>
      ...
      <main className="container section">
        ...
        <div className="test-panel">
          ...
          <p>Paste or type sensitive data below and submit it over HTTP POST. Your DLP should flag this transmission.</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-danger" disabled={sending || !textData.trim()}>
              {sending ? <><span className="spinner" /> Sending...</> : 'Send HTTP POST'}
            </button>
          </form>
        ...
        <div className="test-panel">
          ...
          <p>Upload files containing sensitive data via HTTP POST to test file-based DLP detection.</p>
          ...
          {files.length > 0 && (
            <div className="mt-2">
              ...
              <div className="mt-2">
                ...
                >
                  {uploading ? <><span className="spinner" /> Uploading...</> : 'Upload via HTTP POST'}
                </button>
              </div>
            </div>
          )}
